...
mod tests {
    ...
    fn test_new_and_display() {
        let lin = Lin::new(HashMap::from([(VarId::new(1), Rational::new(1, 2)), (VarId::new(2), Rational::new(-3, 4))]), Rational::from(5));
    ...
    #[test]
    fn test_add_lin() {
        let lin1 = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE)]), Rational::from(2));
        let lin2 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(2)), (VarId::new(2), Rational::from(3))]), Rational::from(4));

        let sum = lin1 + lin2;

        let expected = Lin::new(HashMap::from([(VarId::new(1), Rational::from(3)), (VarId::new(2), Rational::from(3))]), Rational::from(6));
    ...
    fn test_add_rational() {
        let lin = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE)]), Rational::from(2));
        let sum = lin + Rational::from(3);
        let expected = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE)]), Rational::from(5));
    ...
    fn test_sub_lin() {
        ...
        let expected = Lin::new(HashMap::from([(VarId::new(1), Rational::from(-1)), (VarId::new(2), Rational::from(-3))]), Rational::from(-2));
    ...
    fn test_sub_rational() {
        ...
        let expected = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE)]), Rational::from(-1));
    ...
    fn test_mul_rational() {
        let lin = Lin::new(HashMap::from([(VarId::new(1), Rational::new(1, 2))]), Rational::new(3, 4));
        let product = lin * Rational::from(2);
        let expected = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE)]), Rational::new(3, 2));
    ...
    fn test_div_rational() {
        let lin = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE)]), Rational::new(3, 2));
        let quotient = lin / Rational::from(2);
        let expected = Lin::new(HashMap::from([(VarId::new(1), Rational::new(1, 2))]), Rational::new(3, 4));
    ...
    fn test_substitute() {
        // 2*x1 + 3*x2 + 5
        let mut lin1 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(2)), (VarId::new(2), Rational::from(3))]), Rational::from(5));
        // substitute x1 with (4*x3 + 1)
        let lin_sub = Lin::new(HashMap::from([(VarId::new(3), Rational::from(4))]), Rational::ONE);
        lin1.substitute(VarId::new(1), &lin_sub);
        // expected = 3*x2 + 8*x3 + 7
        let expected = Lin::new(HashMap::from([(VarId::new(2), Rational::from(3)), (VarId::new(3), Rational::from(8))]), Rational::from(7));
    ...
    fn test_substitute_combine() {
        // 2*x1 + 3*x2
        let mut lin1 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(2)), (VarId::new(2), Rational::from(3))]), Rational::ZERO);
        // substitute x1 with (x2 + 1)
        let lin_sub = Lin::new(HashMap::from([(VarId::new(2), Rational::ONE)]), Rational::ONE);
        lin1.substitute(VarId::new(1), &lin_sub);
        // expected = 5*x2 + 2
        let expected = Lin::new(HashMap::from([(VarId::new(2), Rational::from(5))]), Rational::from(2));
    ...
    fn test_substitute_merge_and_cancel() {
        // Original: 1x + 2y + 0
        let mut expr = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE), (VarId::new(2), Rational::from(2))]), Rational::ZERO);

        // Substitution: x = -2y + 10
        let sub = Lin::new(HashMap::from([(VarId::new(2), Rational::from(-2))]), Rational::from(10));

        // Result should be: (-2y + 10) + 2y => 0y + 10 => 10
        expr.substitute(VarId::new(1), &sub);
    ...
    fn test_substitute_missing_var() {
        let mut lin = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE)]), Rational::ZERO);
        let lin_sub = Lin::new(HashMap::new(), Rational::ZERO);
        // Panic: Variable 2 not in lin
        lin.substitute(VarId::new(2), &lin_sub);
    }

    #[test]
    fn test_display_edge_cases() {
        // Test with coefficient -1 as first term
        let lin1 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(-1))]), Rational::ZERO);
        assert_eq!(format!("{}", lin1), "-x1");

        // Test with coefficient -1 as non-first term
        let lin2 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(2)), (VarId::new(2), Rational::from(-1))]), Rational::ZERO);
        let s = format!("{}", lin2);
        assert!(s.contains("-x") || s.contains("- x"));

        // Test with negative coefficient (not -1) as first term
        let lin3 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(-3))]), Rational::ZERO);
        assert_eq!(format!("{}", lin3), "-3*x1");

        // Test with positive coefficient in later terms
        let lin4 = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE), (VarId::new(2), Rational::from(3))]), Rational::ZERO);
        let s = format!("{}", lin4);
        assert!(s.contains("+") || s.contains("x1") && s.contains("x2"));

        // Test with coefficient +1 as non-first positive term (line 108)
        let lin_pos_one = Lin::new(HashMap::from([(VarId::new(1), Rational::from(2)), (VarId::new(2), Rational::ONE)]), Rational::ZERO);
        let s = format!("{}", lin_pos_one);
        // Should contain "+ x2" (not "+ 1*x2")
        assert!(s.contains("+ x") || s.contains("+x"));

        // Test with negative coefficient (not -1) as non-first term (line 113)
        let lin_neg = Lin::new(HashMap::from([(VarId::new(1), Rational::from(2)), (VarId::new(2), Rational::from(-5))]), Rational::ZERO);
        ...
        let lin6 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(2))]), Rational::from(3));
        let s = format!("{}", lin6);
        assert!(s.contains("+ 3"));

        // Test with negative constant term
        let lin7 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(2))]), Rational::from(-3));
        let s = format!("{}", lin7);
        assert!(s.contains("- 3"));

        // Test expression with zero constant
        let lin8 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(2))]), Rational::ZERO);
        let s = format!("{}", lin8);
    ...
    fn test_add_lin_variations() {
        let lin1 = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE)]), Rational::from(2));
        let lin2 = Lin::new(HashMap::from([(VarId::new(2), Rational::from(3))]), Rational::from(4));

        // Test Lin + Lin
        let res1 = lin1.clone() + lin2.clone();
        assert_eq!(res1.vars.get(&VarId::new(1)), Some(&Rational::ONE));
        assert_eq!(res1.vars.get(&VarId::new(2)), Some(&Rational::from(3)));
    ...
    fn test_sub_lin_variations() {
        let lin1 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(5))]), Rational::from(10));
        let lin2 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(2))]), Rational::from(3));

        // Test Lin - Lin
        let res1 = lin1.clone() - lin2.clone();
        assert_eq!(res1.vars.get(&VarId::new(1)), Some(&Rational::from(3)));
    ...
    fn test_sub_rational_variations() {
        let lin = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE)]), Rational::from(10));
        let rat_val = Rational::from(3);
        ...
        let res5 = rat_val - lin.clone();
        assert_eq!(res5.vars.get(&VarId::new(1)), Some(&Rational::from(-1)));
        assert_eq!(res5.known_term, Rational::from(-7));

        // Test Rational - &Lin
        let res6 = rat_val - &lin;
        assert_eq!(res6.vars.get(&VarId::new(1)), Some(&Rational::from(-1)));
        assert_eq!(res6.known_term, Rational::from(-7));

        // Test &Rational - Lin
        let res7 = &rat_val - lin.clone();
        assert_eq!(res7.vars.get(&VarId::new(1)), Some(&Rational::from(-1)));
        assert_eq!(res7.known_term, Rational::from(-7));

        // Test &Rational - &Lin
        let res8 = &rat_val - &lin;
        assert_eq!(res8.vars.get(&VarId::new(1)), Some(&Rational::from(-1)));
        assert_eq!(res8.known_term, Rational::from(-7));
    }

    #[test]
    fn test_mul_rational_variations() {
        let lin = Lin::new(HashMap::from([(VarId::new(1), Rational::from(2))]), Rational::from(3));
        let rat_val = Rational::from(4);

        // Test Lin * Rational
        let res1 = lin.clone() * rat_val;
        assert_eq!(res1.vars.get(&VarId::new(1)), Some(&Rational::from(8)));
    ...
    fn test_div_rational_variations() {
        let lin = Lin::new(HashMap::from([(VarId::new(1), Rational::from(8))]), Rational::from(12));
        let rat_val = Rational::from(4);

        // Test Lin / Rational
        let res1 = lin.clone() / rat_val;
        assert_eq!(res1.vars.get(&VarId::new(1)), Some(&Rational::from(2)));
    ...
    fn test_add_assign_variations() {
        // Test AddAssign<Lin>
        let mut lin1 = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE)]), Rational::from(2));
        lin1 += Lin::new(HashMap::from([(VarId::new(2), Rational::from(3))]), Rational::from(4));
        assert_eq!(lin1.known_term, Rational::from(6));

        // Test AddAssign<&Lin>
        let mut lin2 = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE)]), Rational::from(2));
        lin2 += &Lin::new(HashMap::from([(VarId::new(2), Rational::from(3))]), Rational::from(4));
        assert_eq!(lin2.known_term, Rational::from(6));

        // Test AddAssign<Rational>
        let mut lin3 = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE)]), Rational::from(2));
        lin3 += Rational::from(5);
        assert_eq!(lin3.known_term, Rational::from(7));

        // Test AddAssign<&Rational>
        let mut lin4 = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE)]), Rational::from(2));
    ...
    fn test_sub_assign_variations() {
        // Test SubAssign<Lin>
        let mut lin1 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(5))]), Rational::from(10));
        lin1 -= Lin::new(HashMap::from([(VarId::new(1), Rational::from(2))]), Rational::from(3));
        assert_eq!(lin1.vars.get(&VarId::new(1)), Some(&Rational::from(3)));
        assert_eq!(lin1.known_term, Rational::from(7));

        // Test SubAssign<&Lin>
        let mut lin2 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(5))]), Rational::from(10));
        lin2 -= &Lin::new(HashMap::from([(VarId::new(1), Rational::from(2))]), Rational::from(3));
        assert_eq!(lin2.known_term, Rational::from(7));

        // Test SubAssign<Rational>
        let mut lin3 = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE)]), Rational::from(10));
        lin3 -= Rational::from(3);
        assert_eq!(lin3.known_term, Rational::from(7));

        // Test SubAssign<&Rational>
        let mut lin4 = Lin::new(HashMap::from([(VarId::new(1), Rational::ONE)]), Rational::from(10));
    ...
    fn test_mul_assign_variations() {
        ...
        let mut lin1 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(2))]), Rational::from(3));
        lin1 *= Rational::from(4);
        assert_eq!(lin1.vars.get(&VarId::new(1)), Some(&Rational::from(8)));
        assert_eq!(lin1.known_term, Rational::from(12));

        // Test MulAssign<&Rational>
        let mut lin2 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(2))]), Rational::from(3));
    ...
    fn test_div_assign_variations() {
        ...
        let mut lin1 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(8))]), Rational::from(12));
        lin1 /= Rational::from(4);
        assert_eq!(lin1.vars.get(&VarId::new(1)), Some(&Rational::from(2)));
        assert_eq!(lin1.known_term, Rational::from(3));

        // Test DivAssign<&Rational>
        let mut lin2 = Lin::new(HashMap::from([(VarId::new(1), Rational::from(8))]), Rational::from(12));
        lin2 /= &Rational::from(4);
